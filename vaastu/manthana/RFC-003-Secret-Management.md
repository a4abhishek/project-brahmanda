...
### **4. Vault Isolation (Project-Brahmanda)**

- **Problem:** Storing project secrets in the default "Private" vault mixes personal credentials (banking, email) with infrastructure keys, violating the principle of least privilege.
- **Solution:** We will create a dedicated 1Password Vault named **"Project-Brahmanda"**.
- **Benefit:** This allows us to scope Service Account tokens strictly to this vault, ensuring that a compromised CI/CD token cannot access personal data. It also simplifies secret rotation and handover.
...
- **Execution:**
  ...
  ansible-playbook setup.yml \--vault-password-file \<(op read "op://Project-Brahmanda/Ansible Vault/password")
